...
pro box_cur2, x0, y0, nx, ny, INIT = init, FIXED_SIZE = fixed_size, $
	MESSAGE = message
;+
; NAME:
;	BOX_CURSOR
...
while 1 do begin
	...
	button = !err
	dist= float(px(corner)-x)^2 + float(py(corner)-y)^2	; k.i.
...
;	if !err eq 1 then begin  ;Drag entire box?
	if !err eq 1 and dist gt 100 then begin  ;Drag entire box?
		x0 = x00 + x - mx0
		y0 = y00 + y - my0
		endif
;	if (!err eq 2) and (keyword_set(fixed_size) eq 0) then begin ;New size?
	if (!err eq 1) and dist le 100 and (keyword_set(fixed_size) eq 0) then begin ;New size?
