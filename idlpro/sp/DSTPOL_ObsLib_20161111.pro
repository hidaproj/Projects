...
img=congrid(img0,800,600)
;xy1=[xy[0]*2./wp.binx,xy[1]*2./wp.biny]
xy1=[xy[0]*2048./800./wp.binx,xy[1]*2048./800./wp.biny]
...
;	if (wp.input eq  '225°') and (purpose eq '225') then begin
	if (wp.input eq  '135°') and (purpose eq '225') then begin
		print,'225'	&wait,waittime		;wait,5がないと入力信号の重なりが起きる＊ポイント！！
		fn[4]=PolObs('225',wp)
...
;	if (wp.input eq  '270°') and (purpose eq '270') then begin
	if (wp.input eq  '135°') and (purpose eq '270') then begin
		print,'270'	&wait,waittime		;wait,5がないと入力信号の重なりが起きる＊ポイント！！
		fn[5]=PolObs('270',wp)
		cdio_o315,wp,wd	&	purpose='315'
print,'===== 270°終了 ====='
	endif
	if (wp.input eq  '315°') and (purpose eq '315') then begin
		print,'315'	&wait,waittime		;wait,5がないと入力信号の重なりが起きる＊ポイント！！
		fn[6]=PolObs('315',wp)
		cdio_o360,wp,wd	&	purpose='360'
print,'===== 315°終了 ====='
	endif
	if (wp.input eq  '360°') and (purpose eq '360') then begin
		print,'0'	&wait,waittime		;wait,5がないと入力信号の重なりが起きる＊ポイント！！
		fn[7]=PolObs('0',wp)
