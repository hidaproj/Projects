...
while ev_stop.id eq 0 do begin
	...
	dstangl,hangl,zd,radius,pangl,incli,ga,num_grating,im,file_size=file_size
	...
	str1=string(file_size)+'  '+	$
		yyyymmddhhmmss1+', HA '+string(hah,format='(i2.2)')+':'+string(ham,format='(i2.2)')+	$
...
yyyymmddhhmmss=string(year,form='(i4.4)')+string(mon,form='(i2.2)')+string(day,form='(i2.2)') $
	+string(hour,form='(i2.2)')+string(minu,form='(i2.2)')+string(seco,form='(i2.2)')
fnt='HG‘n‰pŠpƒSƒVƒbƒNUB*BOLD*30'
fnt1='HG‘n‰pŠpƒSƒVƒbƒNUB*BOLD*40'
...
bs_dir=widget_base(main, /row)
	lab=widget_label(bs_dir,value='Save Directory : ',FONT=fnt)
	text=widget_text(bs_dir,value=wd.svdir, xsize=30, uvalue='svdir',/edit,FONT=fnt)
bs_con=widget_base(main, /row)
	wd.cont=widget_text(bs_con,value='', xsize=50, ysize=1, uvalue='str',/edit,FONT=fnt1)
;bs_con1=widget_base(main, /row)

;	bt_st=WIDGET_BUTTON(bs_con1,uvalue='close',value='close dstangle.dat ( IDL> close,1 )'$
;	,/align_center,xsize=500,FONT=fnt)
bs_bt=widget_base(main, /row)
	bt_st=WIDGET_BUTTON(bs_bt,uvalue='start',value='Start',/align_center,xsize=250,FONT=fnt)
	wd.bt_en=WIDGET_BUTTON(bs_bt,uvalue='stop',value='Stop',/align_center,xsize=250,FONT=fnt)
	bt_st=WIDGET_BUTTON(bs_bt,uvalue='close',value='close dstangle.dat ( IDL> close,1 )'$
	,/align_center,xsize=500,FONT=fnt)
