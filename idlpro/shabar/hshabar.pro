...
;To convert .sav file, run following comment out lines
;CD,'C:\Projects\idlpro\shabar'
;.Reset_session
;.Compile hshabar
;Resolve_All,skip=['solarpos']
;Save, /Routines, Filename='hshabar.sav'
...
ver='1.0' ; '2016/04/26 k.o.    separate hshabar from PC7, 64bit version
return,ver
end
...
;spawn,'net use t: \delete',/hide
;spawn,'net use t: \\SMARTSYS\command',/hide
...
case (ev.id) of
	...
	while ev_shbstop.id eq 0 do begin
		...
		if dnev eq 1 then begin
			...
			if ndat ge p.n_rec1 then begin
				...
				printf,1,outc
				close,1
				openw,2,'\\SMARTSYS\command\shabar.txt'
				printf,2,outc
				close,2
				ndat=0l
  ...
  wd.Plot: begin
	...
	readu,1,tmpgif
	close,1
  	tmp_ret=call_external('C:\Projects\cprog\vs2010\ftptest\Debug\ftptest.dll','PutData',tmpgif,sstmp.size, $
  						'3.9.0.210','/smart3/shabar/gif/hsb_'+p.date+'.gif','smart','spw!3BHa',/cdecl)
  	print,tmp_ret
	openr,1,csvfile
  	sstmp=fstat(1)
	...
	readu,1,tmplog
	close,1
  	tmp_ret=call_external('C:\Projects\cprog\vs2010\ftptest\Debug\ftptest.dll','PutData',tmplog,sstmp.size, $
  						'3.9.0.210','/smart3/shabar/log/hsb_'+p.date+'.csv','smart','spw!3BHa',/cdecl)
	print,tmp_ret
	end
  wd.Gif: begin
  	giffile=p.gifdir+'hsb_'+p.date+'.gif'
...
caiodll='C:\Projects\cprog\vs2010\aio_64\x64\debug\aio_64.dll'
