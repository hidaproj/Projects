...
pro T3_select_ps1d_process,ev
  ...
  events=nev ne 0?reform(string(strmid(eventlist,[0,2,5,7,10,12],2),form='(a2,":",a2)'),3,nev):''

  ans=dialog_message(I18N_UTF8TOMULTIBYTE('以下の内容でよろしいですか？'+string(13B)+ $
    'events (in JST):'+string(13B)+(nev ne 0?strjoin(strjoin(events,'-'),string(13B)):'none')),title='T3 Select ps1d',/question,dialog_parent=ev.top)
  ...
  for i=0,ns-1 do begin
    ...
    if mean(img1) gt dark_level then begin
      ...
      k++
    endif else widget_control,uval0.textw,set_value=strtrim(i,2)+'/'+strtrim(ns-1,2)+' '+cfi3[iis[k]]+' cloudy...'
  ...
  if nev ne 0 then begin
    ...
    for k=0,nev-1 do begin
      event1=events[*,k]
      text=['event selection : '+strjoin(events[*,k],'-')]
      widget_control,uval0.textw,set_value=text
