...
; '16/07/19 K. Otsuji using timer widget
;-
;*************************************************
pro xmovie_event,ev
  ...
  if (TAG_NAMES(ev, /STRUCTURE_NAME) EQ 'WIDGET_TIMER') then begin
    ...
    delay=wd.Delay(j)
    widget_control,ev.top,timer=delay
    return
  endif 
...
pro xmovie,img0,modal=modal,fit=fit,irange=irange,info=info0, $
  ...
  wd={wd_xmovie, $
    ...
    Exit: 0l, $
    Direction: 0l $ ; +1->Forward, -1->Reverse
  }
  ;base = WIDGET_BASE(title='Xmovie', /column, /modal, /floating, group_leader=gl )
  base = WIDGET_BASE(title='Xmovie', /column ,modal=modal,group_leader=gl)
