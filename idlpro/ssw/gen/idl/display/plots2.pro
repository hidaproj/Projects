pro plots2, x, y, dev=dev, color=color, thick=thick, continue=continue, psym=psym, symsize=symsize
;
;
;	plots2, x, y, /dev
;
...
if (!d.name ne 'PS') then begin
    plots, x, y, dev=dev, color=color, thick=thick, continue=continue, psym=psym, symsize=symsize
end else begin
    xx = x / float(xsiz_pix) * !d.x_size
    yy = y / float(ysiz_pix) * !d.y_size
    plots, xx, yy, /dev, color=color, thick=thick, continue=continue, psym=psym, symsize=symsize
end
;
end
