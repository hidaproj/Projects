...
if exist(input) and exist(append) then begin
 if keyword_set(no_copy) then return,[temporary(input),temporary(append)] else $
  return,[input,append]
endif

if exist(input) and (not exist(append)) then return,input
if (not exist(input)) and exist(append) then return,append
