...
if is_number(plotman) then self.plotman= 0b > plotman < 1b
...
for i=0,nfiles-1 do begin
 self.http->copy,rfile[i],lfile[i],_extra=extra,err=err,/progress,/slurp,$
...
ofiles='' & err='' & count=0 & cancelled=0b
rok=self->is_remote(files,class=class,count=rcount)
if rcount eq 0 then return
doit=where(rok,rcount)
rfiles=files[doit]
rclass=class[doit]
clobber=self->getprop(/clobber)
...
for i=0,rcount-1 do begin
 if (i gt 0) then begin
  if not (strcmp(rclass[i],obj_class(last_robj),/fold)) then begin
   obj_destroy,robj & robj=obj_new(rclass[i])
  endif
 endif else robj=obj_new(rclass[i])
 if obj_valid(robj) then begin
  robj->copy,rfiles[i],out_dir=ldir,_extra=extra,$
...
count=n_elements(tfiles)
if count gt 0 then ofiles=temporary(tfiles)
if obj_valid(robj) then obj_destroy,robj
if obj_valid(last_robj) then obj_destroy,last_robj
...
self->rcopy,cfile,rfiles,_extra=extra,count=count,err=err,cancelled=cancelled

;if cancelled then return,rfiles
rcount=count
...
if count gt 0 then begin 
 ...
 if mode eq 0 then rpatt=replicate('*',count)
...
if (uvalue eq 'apply') or (uvalue eq 'directory') then begin
 if (1-self->getprop(/current)) then begin
  widget_control,info.dtext,get_value=new_dir
  new_dir=trim(new_dir)
  old_dir=trim(self->getprop(/ldir))
  if old_dir ne new_dir then begin
   if not is_dir(new_dir,err=err,out=out_dir) then begin
    xack,err 
    return
   endif
   self->setprop,ldir=out_dir
  endif
 endif
endif
...
case tvalue of
 ...
 'yes_current': begin
    cdir=curdir()
    self->setprop,current=1,ldir=cdir
    widget_control,info.dtext,set_value=curdir()
   end

 'no_current':  self->setprop,current=0
                 
 'browse': begin
   if 1-self->getprop(/current) then begin
    old_dir=self->getprop(/ldir)
    new_dir=dialog_pickfile(/direct,group=event.top)
    if trim(new_dir) eq '' then return
    if old_dir ne new_dir then begin
     if not is_dir(new_dir,err=err,out=out_dir) then begin
      xack,err
      return
...
for i=0,n_elements(sites)-1 do begin
 ...
 reg=abbr+'.+'
 if is_string(patt) then reg=reg+patt
...
pro synop::hsearch,files,times=times,sizes=sizes,count=count,err=err,_ref_extra=extra

;-- search local sites

self->lsearch,files,times=times,sizes=sizes,count=count,err=err,_extra=extra
...
if dcount gt 0 then begin
 ...
 for i=0,dcount-1 do begin
  ...
  if obj_valid(robj) then begin
   robj->find,rfiles,tstart,tend,sizes=rsizes,times=rtimes,count=rcount,$
              cache=cache,/round_day,err=err
   obj_destroy,robj
...
self={synop,synop_dir:'',last_count:0l,mode:0, site:'', wave:'', plotman:0b,$
