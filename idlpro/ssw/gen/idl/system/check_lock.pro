...
if datatype(lock_file) ne 'STR' then return,1

;-- if file doesn't exist, then it has expired

chk=loc_file(lock_file,count=count)
if count ne 1 then begin
 if noisy then message,'LOCK file does not exist',/cont
 return,1
endif
...
if not rok then begin
 ...
 if noisy then message,merr,/cont
; return,0
endif
...
if expired or is_blank(lock_id) then begin
 if noisy then message,'LOCK file has expired',/cont
endif else begin
...
; expired=(lock_id eq user_id) and (user_pid eq lock_pid) and (user_tty eq lock_tty)
 ...
 if not remove then err='Procedure locked by '+lock_id+' at '+lock_date+' on TTY '+lock_tty else $
  if noisy then message,'LOCK file can be removed by current user',/cont
endelse

if noisy and (err ne '') then message,err,/cont
time=utc
return,remove
