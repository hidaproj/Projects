...
function test_dir,dir_name,out=out,quiet=quiet,err=err,count=count,_extra=extra
...
if np gt 1 then begin
 ...
 for i=0,np-1 do begin
  bool[i]=test_dir(dir_name[i],out=tout,quiet=quiet,err=temp,_extra=extra)
  terr[i]=trim(temp)  
  out[i]=tout
 endfor
...
if not is_dir(dir_name,out=out,_extra=extra) then begin
...
if not write_dir(dir_name,_extra=extra) then begin
