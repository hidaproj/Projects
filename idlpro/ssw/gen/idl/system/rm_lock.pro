...
if datatype(file) eq 'STR' then lock_file=file else return
clook=loc_file(lock_file,count=count)
if count eq 0 then begin
 if noisy then message,'LOCK file already removed',/cont
 return
endif
...
repeat begin
 ...
 if (twait gt 0) and (err ne '') then begin
  if noisy then message,'Waiting for file to be unlocked...',/cont
  wait,twait
 endif
endrep until ((err eq '') or (twait eq 0) or (k gt retries))

if (err eq '') and noisy then message,'Removed LOCK file - '+lock_file,/cont
