...
function write_dir,name,out=out,valid_dir=valid_dir,_ref_extra=extra
...
for i=0,np-1 do begin
 valid_dir[i]=is_dir(name[i],out=tout,_extra=extra)
 if valid_dir[i] then begin
  access[i]=test_open(tout,/write,_extra=extra)
  out[i]=tout
 endif
endfor
