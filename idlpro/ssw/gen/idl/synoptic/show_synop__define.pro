...
;-- get GOES times 
 ...
 if (button_val eq 'Plot GOES') then begin
...
guig=widget_button(right,value='Plot GOES',font=bfont,event_pro='plot_goes_eh')
...
;  widget_control,info.main,sensitive=1
 ...
 if (bname eq 'change') or (bname eq 'directory') then begin
  ...
  if bname eq 'change' then $
   new_dir=dialog_pickfile(/directory,/must_exist,dialog_parent=event.top) else $
...
;   automatic color scaling
   ...
   if plot_type eq 'image' then begin
    log_scale=self->is_log(file_id)
    if log_scale then data->set,log_scale=log_scale
    data->set,load_color=(1-use_plotman)
   endif
