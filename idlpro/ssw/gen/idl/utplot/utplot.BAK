...
if atime_format eq 'YOHKOH' and NOT keyword_set(nolabel) then begin
...
;           LABEL = 'Start Time (' + TLABW   + ')'       ;MDM remove the
           if keyword_set( date_label_only ) then begin ;date label by csillag 5-apr-2004 
               TLABW =  strmid(LABW,0,9)
               LABEL = 'time (' + TLABW   + ')' ; arnold benz' format
           endif else begin 
               TLABW =  strmid(LABW,0,18)
               LABEL = 'Start Time (' + TLABW   + ')' ;MDM remove the
...
if nwrange lt 1 then begin
	message,/continue,'Error, Times passed out of plotting range.  ' 
	message,'Time-range of plot box: '+ atime(!x.crange(0) + utbase)+ ' '+$
		atime(!x.crange(1) + utbase),/continue
	message,'Time-range of data passed: '+atime( (minmax(x))(0) + utbase) + ' '+$
		atime( (minmax(x))(1) + utbase),/continue
endif 
