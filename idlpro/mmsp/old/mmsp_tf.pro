...
;**************************************************************************
pro mmsp_TF_event, ev1
common mmsp_com, wd, p, wl, sp1, drk, dat, clr, outfile0, npix,ptemp
...
;		temp=get_temp(/text)
	...
	print,p.T_mode
	if p.T_mode eq 'On' then begin
;		p.temp=get_temp(/init,/text)	; init contex AD
		get_lctemp2,t,v,ptemp,time,/init
		p.temp=t
		widget_control, wd.T_get, set_value=p.temp
	endif
	end
