...
if p.T_mode eq 'On' then p.temp=get_temp(/init,/text)	; init contex AD
...
;	setlcvolt,lcvinit,-1	; ARCOptics driver
	...
	for k=0,nv-1 do begin
	    ...
	    if p.T_mode eq 'On' then p.temp=get_temp(/text)
	    ...
	    if p.T_mode eq 'On' then p.temp=p.temp+', '+get_temp(/text)
