...
ver='0.9'	; 2016.07.19   k.i.,  MM[*,*,i]=MM[*,*,i]#MMCR[*,*,i],  wd.pg90
return,ver
end
...
case (ev1.id) of
	...
	if 1 then begin
		...
		plot,wl[iiv],c[iiv].ret1,ytitle='retardation [deg.]',xtickname=replicate(' ',10), $
			charsize=1.5,title='MMSP waveplates',yrange=[80,160],pos=[0.15,0.5,0.95,0.9]
		oplot,wl[iiv],c[iiv].ret2,line=2
		plot,wl[iiv],c[iiv].offset1-ofm1,xtitle='wavelength [nm]',ytitle='axis [deg.]', $
			charsize=1.5,yrange=90.*[-1,1],pos=[0.15,0.1,0.95,0.497],/noerase
		oplot,wl[iiv],c[iiv].offset2-ofm2,line=2
	endif
	mmsp_mueller,th1,th2,cdat,c,MMC,fit=fitv	; 2016.7.19
	MMCR=MMC
	for i=0,n_elements(wl)-1 do MMCR[*,*,i]=invert(MMC[*,*,i])
	end
     wd.Fileb: begin
	...
	imgsize,dat,nw,nn
	;--------------  Mueller matrix  ----
	mmsp_mueller,th1,th2,dat,c,MM,fit=fitv,pg90=wd.pg90
	for i=0,n_elements(wl)-1 do MM[*,*,i]=MM[*,*,i]#MMCR[*,*,i]	; 2016.7.19
	...
	mmplot,wl,mm,xtitle='wavelength [nm]',title=title,xr=xrange,chs=1.2, $
	...
	;	ytitle='retardation',charsize=chars,ystyle=1,yticks=4,yrange=[90,180.]
	yp=y0+dy1*1
...
;	oplot,wl,th,line=1
	...
	chars=1.5
	yp=y0+dy1*1
	plot,wl,ret,pos=[x0,yp,x1,yp+dy1-0.005],xrange=xrange,xstyle=1,xtickname=blank, $
		ytitle='!7D!3ret. [deg.]',charsize=chars,ystyle=1,$ ; yticks=4, $
		;yrange=ret0+rrange*[-1,1], $
		yrange=[ret_mn,ret_mx], $
		title=o.file
	...
	oplot,xrange,0*[1,1],line=1
     ...
     wd.Rpg90: begin	; 2016.7.19
	widget_control,wd.Rpg90,get_value=pg90
	wd.pg90=pg90
	print,'pg90 set ',wd.pg90
	mmsp_mueller,th1,th2,dat,c,MM,fit=fitv,pg90=wd.pg90
	for i=0,n_elements(wl)-1 do MM[*,*,i]=MM[*,*,i]#MMCR[*,*,i]	; 2016.7.19
	s=mmdecmp2(MM,iiv=where(c.valid eq 1),flip=wd.flip)
	ii=where(s.Dia gt 0.97, count)
	...
	win2gif,'c:\tmp\a.gif'
     ...
     wd.OUTCSV: begin
	csvfile='c:\tmp\mmsp_cal.csv'
	openw,1,csvfile
	printf,1,'wl, ret, dia, ang'
	for i=0,n_elements(wl)-1 do  $
		printf,1,wl[i],ret[i],dia[i],ang[i],form='(f10.2,",  ",f10.4,", ",f10.4," ",f10.4)'
	close,1
	print,'wl, ret, dia, ang  saven in '+csvfile
	end
     wd.Exit: begin
...
b3a2b=widget_base(b1, /row )
	wd.Rpg90 = cw_bgroup(b3a2b,['0','1'],/row,label_left='PG rot 90: ', $
		uvalue="Rpg90",/no_release,ypad=0,set_value=wd.pg90,/exclusive,/frame)
