...
ver='0.93'	; '2014/04/28	M.H., get_therm_temp comment out
ver='0.94'	; '2014/06/12	K.S., shut_cnt,val <- hr_gpio
return,ver
end
...
;	sp=dblarr(2048)
    ...
    wd.ShOpn: shut_cntl,iobit,0	; bit-iobit, open
    wd.ShCls: shut_cntl,iobit,1	; bit-iobit, close
;    wd.ShOpn: hr_gpio,iobit,0	; bit-iobit, out-1
;    wd.ShCls: hr_gpio,iobit,1	; bit-iobit, out-0
    wd.Dark: begin
	print,'Get Dark'
;	hr_gpio,iobit,1	; bit-iobit, out-0
        shut_cntl,iobit,1	; bit-iobit, close
;	sp=dblarr(2048)
	...
	drk=sp
        shut_cntl,iobit,0	; bit-iobit, open
...
;*****************************************************
pro shut_cntl,iobit,val,c_dev
;-----------------------------------------------------
; shutter contolor with emp400
;  iobit  -- Number of Out port 1 - 6
;  val    -- 0 = open  or 1 = close
;  c_dev -- 1:EMP400 or 0:grating

if not keyword_set(iobit) then iobit = 1
if not keyword_set(c_dev) then c_dev = 1

if c_dev eq 1 then begin
	if val eq 0 then eout_re = '1'		;open
	if val eq 1 then eout_re = '0'		;close
	eout_no = string(iobit,form='(i1)')

	empcom,'OUT '+eout_no+' '+eout_re
	wait,1.
endif else begin
	hr_gpio,iobit,val
endelse
