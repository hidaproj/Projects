...
function version
  ...
  ver='4.1' ; '2016/12/06 k.o.  count -> lcount (long) in wd.OBS_START and wd.DWset'
  ver='4.2' ; '2016/12/08 k.o.  common bridge for paralell fits output'
  return,ver
end
...
pro utfobs_event, ev
  ...
  case (ev.id) of
    ...
    wd.OBS_START: begin
      ...
      wait,1.  ;for safty finishing orca_capturemode_sequence,nimg=o.nimg (when nimg is large)
      ...
      while ev.id ne wd.OBS_STOP do begin
        ...
        dwl1=o.dvs[lcount mod o.ndw]/3e5*f.wl0
        f.mode=o.fmodes[lcount mod o.ndw]
        ...
        date_obs=get_systime(ctime=ctime,msec=msec)
        if (lcount gt 0) and ((lcount mod o.ndw) eq 0) then begin
          date_obs=get_systime(ctime=ctime,msec=msec)
        ...
        widget_control,wd.FILENAME,set_value=o.filename
        outfile=o.outdir+o.filename+'.fits'
        xyouts,10,p.Height/dbin-30,string(lcount,form='(i5)')+'  '+outfile+'   '+string(dwl1,form='(f6.3)')+'nm',/dev
        savefits_pf,imgs,p,f,file=outfile
        lcount=lcount+1
    ...
    wd.DWset: begin
      ...
      while not eof(1) do begin
        readf,1,str1
        if lcount eq 0 then strs=str1 else strs=[strs,str1]
        lcount=lcount+1l
      endwhile
      close,1
