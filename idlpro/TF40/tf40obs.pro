...
ver='3.0' 	; '2016/03/23 	k.o.	   	from tfobs.pro
...
pro utfobs_event, ev
  ...
  ;-----  go to UTF or ORCA event handler -----
  ...
  ;if n_elements(wdid_f) eq 0 then evid_f=wdevid(wdf)
  ;ii=where(ev.id eq evid_f, count)
  ...
  case (ev.id) of
    ...
    wdf.WSCAN: begin
      ...
      ; tim=get_systime(ctime=ctime)
      ; prof=prof/max(prof)
      ; window,4,xs=512,ys=512
      ; plot,pdwl,prof,$
      ;         xtitle='wavelength [nm]',ytitle='normalized intensity',$
      ;          title=tim+' '+string(f.wl0,form='(f7.2)')+' '+string(mean(f.b.temp),form='(f5.2)')+'C
      ; g0=gaussfit(pdwl,prof,coeff)
      ; x=pdwl
      ; y=g0
      ; b=1000.
      ; nx=n_elements(x)-1
      ; xx=x[0]+dindgen(nx*b+1)/(b*100.)
      ; r=interpol(y,x,xx)
      ; g0=gaussfit(xx,r,coeff)
      ; oplot,xx,r,line=2
      ; lc=coeff[1]
      ; lc=-0.019
      ; oplot,[1,1]*lc,[-0.1,1.2],line=2
      ; print,'line center:',lc
      ; xyouts,lc+wdf.p.wscan.dwstep,0.5,string(lc,form='(f7.4)')+'nm',charsize=1.5,/data
      ; print,'Accept this? : (y/n)'
      ;ans=''
      ;read,ans
      window,4,xs=512,ys=512
      plot,pdwl,prof,$
    ...
    wdf.AUTOADJ: begin
      ...
      restore,f.tbldir+'\f.sav'
      print,'Auto adjust completed!'
    end
   wdf.MODE: begin
    ...
    wd.SCANGET: begin
      ...
      for i=0,nstep-1 do begin
        dwl=wdf.p.wscan.dwmin+i*wdf.p.wscan.dwstep
        obstime[i]=get_systime()
        if i eq 0 then dmy=orcaobs(nimg=1,/noread) $
        else orca_capture
        utf_set_wl,f,dwl=dwl,wait=0.
	dmy=get_systime(msec=ms0)
        imgs[*,*,i]=orca_getimg(nimg=1,/nowait)
        j=(dwl-wdf.p.sld.dwmin)/wdf.p.sld.dwstep
        widget_control, wdf.DWSLD, set_value=j
        widget_control, wdf.DWTXT, set_value=string(f.dwl,form='(f7.3)')
	dmy=get_systime(msec=ms1)
	while ms1-ms0 lt 100 do dmy=get_systime(msec=ms1)	; wait LCVR response
      endfor    
    ...
    wd.OBS_START: begin
      ...
      while ev.id ne wd.OBS_STOP do begin
        ...
        ;print,count,o.ndw,count mod o.ndw
        date_obs=get_systime(ctime=ctime,msec=msec)
