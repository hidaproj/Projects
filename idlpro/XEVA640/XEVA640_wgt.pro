...
; 2016.11.22 T. Anan                Wait after saving data
;-
;*****************************************************
...
; 2014.10.23 T. Anan & T. Yoneya & T. Fujita =>
	...
	wd.obs.wait: BEGIN
		widget_control, eve.id, get_value=value, set_value=''
		h.wait = fix(value[0])
		widget_control, eve.id, set_value = string(h.wait, format='(i4)')
		print, '-- Waiting : '+ string(h.wait)
	RETURN
	END
;<=
...
; 2014.10.23 T. Anan & T.Yoneda & T. Fujita =>
	...
	b_cds3 = widget_base(b_prm, /row)
		lab = widget_label(b_cds3, value='Waiting : ', font='Arial')
		wd_obse.wait = widget_text(b_cds3 $
			,value=string(h.wait, format='(i4)'), uvalue='waite', xsize=4,font='Arial',edit=1)
		lab = widget_label(b_cds3, value=' sec', font='Arial')
;; <=
