...
;		print, '-- C-mode starts Sequences.'
		FOR seqn=0,p.seqno-1 DO BEGIN
			obs_cmode, h=h, img, index=wd.obs.windex, seqn=seqn+1, aseqn=p.seqno, nx, ny;, expo=expo
		...
		WHILE (uvalue1 NE "Tmode_con_stop") DO BEGIN
			...
			CASE uvalue1 OF
				"Tmode_con_start": BEGIN
					...
					endelse
				END
				'Tmode_con_stop': GOTO, loopend
			ENDCASE
		ENDWHILE
		loopend:
		print, '-- T-mode finished ALL Continuous.'
	END
...
; 2014.10.23 T. Anan & T.Yoneda & T. Fujita =>
		...
		wd_obse.Tmode_con_start = widget_button(b_btn1_2, value="Start", uvalue="Tmode_con_start", font='Arial')
		wd_obse.Tmode_con_stop = widget_button(b_btn1_2, value="Stop", uvalue="Tmode_con_stop", font='Arial')
